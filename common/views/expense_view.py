...
def dash(request, menu_id):
    ...
    for r_idx, monthly_year_info in enumerate(monthly_year_list):
        ...
        print(monthly_year_info[0])
        print(monthly_year_info[0])

    menu = Menu.objects.get(id=menu_id)

    context = {'menu': menu, 'top10_label': top10_label, 'top10_data': top10_data, 'top10_sum': top10_sum, 'avg_data': avg_data,
    ...
    print(context)
    return render(request, 'common/expenseDash1.html', context)
...
def get_yearAvg_info():
    sql_str = "SELECT ROUND(AVG(DT.APV_SUM_AMT), 0) "
    sql_str += "    FROM ( "
...
def get_monSum_info():
    sql_str = "SELECT 	TRUNCATE(SUM(APV_SUM_AMT), 0) AS APV_SUM_AMT "
    sql_str += "    FROM cjfv_oneexp.EX_CORPCARD_ASK "
    sql_str += "WHERE SEND_DIV = '01' "
    sql_str += "    AND SLIP_YY = '2021' "
